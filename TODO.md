...
- [ ] **Duplicate destination choice** -- Investigate creating a "Duplicates" folder in the source directory instead of sending to Recycle Bin. Recycle Bin has size limits and may silently purge files. A local folder is visible, browsable, and the user can delete at their own pace. Could also offer choice: Duplicates folder (default) vs Recycle Bin vs user-chosen folder via browse picker. When sending files home or finalizing, confirm where duplicates should go. "Other folder" option lets users park duplicates somewhere for one last look before manually deleting. Addresses the risk of Recycle Bin hitting its size limit (defaults to 10% of drive) and silently purging oldest files — which could include the duplicates the user just sent there. The browse picker already exists. Apply to both finalize and Send Files Home flows. Files: web/finish.js, web/dashboard.js.
